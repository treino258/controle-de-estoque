...
def get_dashboard_estoque(session: Session) -> list[dict]:
    produtos = session.query(Product).filter(Product.ativo.is_(True)).all()
    resultado = []
...
def delete_product(session: Session, product_id: int) -> None:
    ...
    if produto.movements:
        raise ValueError("Não é possível excluir produto com movimentações.")

    if produto.lots:
        raise ValueError("Não é possível excluir produto com lotes.")
